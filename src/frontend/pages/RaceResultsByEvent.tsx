...
export default function RaceResultsByEvent() {
  ...
  // Find the race event from presets by eventId
  const race = useMemo(() => {
    return RACES.find(r => r.id === eventId) ?? undefined
  }, [eventId])
  ...
  useEffect(() => {
    ...
    fetch(`/api/results/race/${race.round}`, { headers })
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) {
  ...
  }, [race, setBackgroundConfig, session?.access_token])
  ...
  // If eventId doesn't match any race, redirect to season overview
  if (!race && !loading) {
