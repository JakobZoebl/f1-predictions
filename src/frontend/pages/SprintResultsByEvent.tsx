...
import { DRIVERS, SPRINTS } from "@/lib/f1-presets"
...
export default function SprintResultsByEvent() {
  ...
  // Find the sprint event from presets by eventId
  const sprint = useMemo(() => {
    return SPRINTS.find(s => s.id === eventId) ?? undefined
  }, [eventId])
  ...
  useEffect(() => {
    ...
    fetch(`/api/results/sprint/${sprint.round}`, { headers })
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) {
  ...
  }, [sprint, setBackgroundConfig, session?.access_token])
  ...
  // If eventId doesn't match any sprint, redirect to season overview
  if (!sprint && !loading) {
    return <Navigate to="/season-overview" replace />
  }
