...
from api._utils.supabase_client import get_supabase_client, execute_with_retry
...
def get_season_stats():
    ...
    try:
        return execute_with_retry(_get_season_stats_inner)
    except Exception as e:
        print(f"Error in get_season_stats: {str(e)}")
        return jsonify({"success": False, "error": str(e)}), 500

def _get_season_stats_inner():
    """Inner logic for get_season_stats, wrapped with retry."""
    user = g.current_user
    supabase = get_supabase_client()
...
def get_cards_stats():
    ...
    try:
        ...
        return execute_with_retry(lambda: _get_cards_stats_inner(team_id, driver_id))
    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({"success": False, "error": str(e)}), 500

def _get_cards_stats_inner(team_id, driver_id):
    """Inner logic for get_cards_stats, wrapped with retry."""
    supabase = get_supabase_client()
...
def _get_profile():
    ...
    try:
        return execute_with_retry(_get_profile_inner)
    except Exception as e:
        return jsonify({"success": False, "error": f"Server error: {str(e)}"}), 500

def _get_profile_inner():
    """Inner logic for _get_profile, wrapped with retry."""
    user = g.current_user
    supabase = get_supabase_client()
